...
export const state = {
  books:        [],
  connections:  [],            // inizializzato in main.js da localStorage o defaults
  statusFilter: 'all',         // 'all' | 'read' | 'reading' | 'unread'
  authorFilter: null,
  sortOrder:    'theme',       // 'theme'|'title'|'title_desc'|'author'|'rating'|'recent'
  view:         'shelf',
  searchQuery:  '',
  currentBookId: null,
};
...
// ─── DEFAULT CONNECTIONS (hardcoded, migrati in localStorage al primo avvio) ──
...
export function getSortedBooks(arr) {
  ...
  return [...arr].sort((a, b) => {
    ...
    if (s === 'recent')     return (b.dateAdded || 0) - (a.dateAdded || 0);
    return 0;
  });
}
